...
generatedSignatureTip: "当前签字预览如下。重新打开页面时会显示已保存的签字效果。",
 passportReminderTitle: "护照信息辅助提醒", passportChecking: "正在识别护照信息并与申请表进行比对，请稍候...", passportReminderDesc: "该提醒仅供人工复核参考，不影响保存草稿或提交申请。", passportCompareFormValue: "申请表", passportCompareOcrValue: "OCR识别", passportNameMismatch: "护照姓名与申请表姓名不一致", passportNoMismatch: "护照号与申请表填写不一致", passportNoLikelyMatch: "护照号疑似一致，但 OCR 可能把 1/I、0/O 等字符识别混淆，请人工确认", passportBirthMismatch: "出生日期与护照信息不一致",
      materialOnlyBanner: "当前为补材料模式：仅允许修改退款账户信息、财政担保信息与上传申请材料，前半部分申请信息已锁定。", openIntake: "当前开放批次", noOpenIntake: "当前没有开放批次", submitIntake: "本次提交批次", noSelectedIntake: "暂未选择，当前可先保存草稿", noIntakeHint: "当前没有开放批次，但仍可继续填写并保存草稿。正式提交时再选择批次即可。",
    ...
    options: {
      ...
      signatureMethods: [{ value: "auto", label: "系统自动签字" }, { value: "draw", label: "手写签字" }, { value: "upload", label: "上传签字图片" }],
...
function MaterialUploadCard({
  ...
  {hasNewFiles ? (
    <>
      <div className="mb-2 text-sm font-semibold text-slate-700">{t.common.selectedFiles}</div>
      <ul className="space-y-2 text-sm text-slate-700">
...
function NewLanguageApplicationPage() {
...
const LOCK_TIMEOUT_MINUTES = 20;
  ...
  ? `${window.location.origin}/student/language-application/${studentFillToken}`
  : "";
  ...
  const materialItems = useMemo(() => {
    return [
      ...
      },
      {
        ...
        accept: ".pdf,.jpg,.jpeg,.png",
        statusType: "exempt",
        statusLabel: t.materialStatus.exempt,
      },
      {
  key: "personalStatement",
  label: t.materials.personalStatement.label,
  desc: t.materials.personalStatement.desc,
  required: true,
  uploadable: false,
  multiple: false,
  accept: ".pdf,.doc,.docx",
...
}, [inKorea, financialGuaranteeRequired, t]);
...
const getPassportCompareDisplayValue = (value) => {
  const normalized = String(value || "").trim();
  if (normalized) return normalized;

  if (language === "en") return "Empty";
  if (language === "ko") return "비어 있음";
  return "空";
};

const buildMismatchMessage = (message, formValue, ocrValue) => {
  return `${message}：${t.common.passportCompareFormValue} ${getPassportCompareDisplayValue(
    formValue
  )}，${t.common.passportCompareOcrValue} ${getPassportCompareDisplayValue(
    ocrValue
  )}`;
};
...
const buildPassportWarnings = (ocrResult, currentForm) => {
  ...
  if (ocrPassportNo && formPassportNo !== ocrPassportNo) {
    if (
      formPassportNo &&
      formPassportNoLoose &&
      ocrPassportNoLoose &&
...
export default NewLanguageApplicationPage;
