...
function validateUploadFile(file, t) {
  ...
  if (isImage && file.size > IMAGE_MAX_SIZE) {
    if (t?.common?.imageTooLarge) {
      return t.common.imageTooLarge.replace("{fileName}", fileName);
    }
    return `Image file "${fileName}" exceeds 2MB. Please compress it and upload again.`;

  }

  if (isPdf && file.size > PDF_MAX_SIZE) {
    if (t?.common?.pdfTooLarge) {
      return t.common.pdfTooLarge.replace("{fileName}", fileName);
    }
    return `PDF file "${fileName}" exceeds 4MB. Please compress it and upload again.`;
    ...
    <ul className="space-y-2 text-sm text-slate-700">
      {existingFiles.map((file, index) => (
        ...
        >
          ...
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 [&>div:first-child]:hidden">
  {hasNewFiles ? (
    <>
        ...
        {files.map((file, index) => (
          ...
          >
            ...
            <button
              ...
              className="shrink-0 rounded-lg bg-red-50 px-3 py-1 text-[0] font-semibold text-red-600 hover:bg-red-100"
            >
              删除
...
const removeExistingUploadedFile = async (fileType, fileRow) => {
  try {
    ...
    if (!targetPublicId) {
      alert(
        language === "en"
          ? "Missing application identifier. Unable to delete the uploaded file."
          : language === "ko"
          ? "지원 식별자가 없어 업로드된 파일을 삭제할 수 없습니다."
          : "缺少申请标识，无法删除已上传文件。"
      );
      return;
    }
    ...
    const confirmed = window.confirm(
      language === "en"
        ? `Are you sure you want to delete the uploaded file "${fileRow.file_name || "Unnamed file"}"?`
        : language === "ko"
        ? `???? ?? "${fileRow.file_name || "?? ?? ??"}" ?(?) ?????????`
        : `???????????${fileRow.file_name || "?????"}???`
    );
    ...
    alert(
      language === "en"
        ? `Failed to delete uploaded file: ${error.message}`
        : language === "ko"
        ? `업로드된 파일 삭제에 실패했습니다: ${error.message}`
        : `删除已上传文件失败：${error.message}`
    );
    return;
  }
};
