...
generatedSignatureTip: "当前签字预览如下。重新打开页面时会显示已保存的签字效果。",
 passportReminderTitle: "护照信息辅助提醒", passportChecking: "正在识别护照信息并与申请表进行比对，请稍候...", passportReminderDesc: "该提醒仅供人工复核参考，不影响保存草稿或提交申请。", passportCompareFormValue: "申请表", passportCompareOcrValue: "OCR识别", passportNameMismatch: "护照姓名与申请表姓名不一致", passportNoMismatch: "护照号与申请表填写不一致", passportNoLikelyMatch: "护照号疑似一致，但 OCR 可能把 1/I、0/O 等字符识别混淆，请人工确认", passportBirthMismatch: "出生日期与护照信息不一致",
    ...
    options: {
      ...
      admissionTypes: [{ value: "Freshman", label: "新入" }, { value: "Transfer (2nd Semester)", label: "插班第二学期" }, { value: "Transfer (3rd Semester)", label: "插班第三学期" }],
...
fillNameFirst: "Please enter the name first to generate the auto signature", noSignaturePreview: "No signature preview", drawSignatureDesc: "Please sign in the area below.", clearSignature: "Clear Signature", uploadSignatureImage: "Upload Signature Image", generatedSignatureTip: "The current signature preview is shown below. Saved signatures will appear again when you reopen this page.",
 ...
 residenceOptions: [{ value: "abroad", label: "Living Abroad" }, { value: "korea", label: "Living in Korea" }], admissionTypes: [{ value: "Freshman", label: "Freshman" }, { value: "Transfer (2nd Semester)", label: "Transfer (2nd Semester)" }, { value: "Transfer (3rd Semester)", label: "Transfer (3rd Semester)" }], programTracks: [{ value: "Korean Track", label: "Korean Track" }, { value: "English Track", label: "English Track" }, { value: "Bilingual Program (Chinese)", label: "Bilingual Program (Chinese)" }], agreeOptions: [{ value: "agree", label: "I agree" }, { value: "disagree", label: "I disagree" }], acknowledgeOptions: [{ value: "yes", label: "I acknowledge" }, { value: "no", label: "Not confirmed" }] },
...
function Select({ label, value, onChange, options = [], required = false, t = messages.zh }) {
  return (
    <div>
      <Label required={required}>{label}</Label>
      <select
        ...
        className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"
      >
        <option value="">{t?.common?.select || messages.zh.common.select}</option>
        {options.map((option) => {
...
function NewGraduateApplicationPage() {
...
const LOCK_TIMEOUT_MINUTES = 20;
  ...
  ? `${window.location.origin}/student/graduate-application/${studentFillToken}`
  : "";
...
const getCurrentOpenIntakeLabel = (intake) => {
  ...
  if (year && month && round) {
    if (language === "en") return `${year}-${month} Round ${round} Graduate School`;
    if (language === "ko") return `${year}년 ${month}월 ${round}차 대학원`;
    return `${year}年${month}月 第${round}批大学院申请`;
  }
...
if (lockError) throw lockError;
  ...
  statusLabel: t.materialStatus.generated,
    ...
    }, [bilingualTrack, inKorea, financialGuaranteeRequired, t]);
...
export default NewGraduateApplicationPage;
